...
def generate_images(
    ...
    logger.success("GPU JPEG encoder initialized")
    ...
    if s3_bucket:
        ...
        logger.info(
            "S3 upload enabled: s3://{}/{} (8 parallel workers)",
            s3_bucket_name,
            s3_prefix,
        )
    ...
    if G.c_dim != 0:
        ...
        if class_idx is not None:
            logger.warning(
                "--class=lbl ignored when running on an unconditional network"
            )
    ...
    if weight_vector is not None:
        ...
        logger.info(
            "Applying weight vector to style blocks {} to {} (inclusive)",
            start_idx,
            end_idx,
        )
    ...
    if weight_vector is not None:
        if vgg_path is None:
            ctx.fail(
                "--vgg-path is required when using --weight-vector for age filtering"
            )
        ...
        if weight_vec is not None:
            ...
            if age_predictor is not None:
                ...
                for i, seed in enumerate(batch_seeds):
                    ...
                    if estimated_age is None:
                        ...
                        logger.warning(
                            "  Seed {}: Age {:.1f} <= 20, skipping...",
                            seed,
                            estimated_age,
                        )
                    else:
                        logger.success(
                            "  Seed {}: Age {:.1f} > 20, proceeding...",
                            seed,
                            estimated_age,
                        )
            ...
            if len(valid_indices) > 0:
                ...
                for alpha in alphas:
                    ...
                    w_modified_batch[:, start_idx : end_idx + 1, :] += (
                        alpha * weight_vec[start_idx : end_idx + 1, :].unsqueeze(0)
                    )
                    ...
                    jpeg_list = gpu_jpeg_encoder.encode([img for img in imgs], ".jpg")
                    ...
                    for i, seed in enumerate(valid_seeds):
                        ...
                        if upload_executor:
                            ...
                            future = upload_executor.submit(
                                upload_to_s3_async, jpeg_list[i], s3_key
                            )
                ...
                logger.info(
                    "  Total alpha generation: {:.2f}s for {} alphas × {} seeds = {} images",
                    alpha_generation_time,
                    len(alphas),
                    len(valid_seeds),
                    len(alphas) * len(valid_seeds),
                )
            ...
            imgs = G.synthesis(
                w_batch, truncation_psi=truncation_psi, noise_mode=noise_mode
            )
            imgs = (
                (imgs.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
            )
    ...
    if upload_executor and upload_futures:
        ...
        logger.success(
            "S3 uploads completed: {}/{} successful in {:.2f}s",
            success_count,
            len(upload_futures),
            upload_time,
        )
        logger.info(
            "   Upload throughput: {:.2f} images/second", success_count / upload_time
        )

        if failed_uploads:
            logger.warning("Failed uploads ({}):", len(failed_uploads))
            for error in failed_uploads[:5]:
    ...
    logger.info(
        "  Average time per image: {:.2f}s", total_time / total_images_generated
    )
    logger.info(
        "  Throughput: {:.2f} images/second", total_images_generated / total_time
    )
    ...
    if create_composite and weight_vec is not None and len(all_images) > 0:
        ...
        if s3_client:
            ...
            try:
                ...
                logger.success(
                    "  Composite uploaded to s3://{}/{}", s3_bucket_name, s3_key
                )
            except Exception as e:
